...
.data
 ...
 mensaje db 10, 13, "Primer numero (uno o dos digitos): $"
 mensaje2 db 10, 13, "Segundo numero (uno o dos digitos): $"
...
main proc
 ...
 ; Solicitar primer n?mero (uno o dos d?gitos)
 mov ah, 09h
 lea dx, mensaje
 int 21h
 ...
 ; Leer el segundo d?gito (opcional) del primer n?mero
 mov ah, 01h
 int 21h
 cmp al, 0Dh ; Comprobar si es 'Enter' (si solo se ingresa un d?gito)
 je primer_numero_finalizado ; Si es Enter, solo se usa el primer d?gito
 sub al, 30h ; Convertir car?cter a n?mero
...
jmp solicitar_segundo_numero

primer_numero_finalizado:
 mov num1, bl ; Si solo se ingres? un d?gito, guardar en num1

solicitar_segundo_numero:
 ; Solicitar segundo n?mero (uno o dos d?gitos)
 mov ah, 09h
 lea dx, mensaje2
 int 21h
 ...
 sub al, 30h ; Convertir car?cter a n?mero
 mov bl, al ; Guardar el primer d?gito en BL
 ; Leer el segundo d?gito (opcional) del segundo n?mero
 mov ah, 01h
 int 21h
 cmp al, 0Dh ; Comprobar si es 'Enter' (si solo se ingresa un d?gito)
 je segundo_numero_finalizado ; Si es Enter, solo se usa el primer d?gito
 sub al, 30h ; Convertir car?cter a n?mero
 mov bh, al ; Guardar el segundo d?gito en BH
...
jmp realizar_operaciones

segundo_numero_finalizado:
 mov num2, bl ; Si solo se ingres? un d?gito, guardar en num2
...
mostrar_cero_mul:
 ; Mostrar "0" para la multiplicaci?n
 mov dl, '0'
 mov ah, 02h
 int 21h

mostrar_cero_div:
 ; Mostrar "0" para la divisi?n
 mov dl, '0'
 mov ah, 02h
 int 21h
